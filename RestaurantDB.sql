...
UPDATE ORDER_DATA -- UPDATES ORDER_DATA.totalCost attribute given specific orderID
...
UPDATE REWARDS_MEMBER m, ORDER_DATA d -- UPDATES REWARDS_MEMBER.rewardsPoints based on their order total * 10 
...
UPDATE DAILY_STATS -- UPDATES DAILY_STATS.dailyIncome attribute given specifc orderDate
SET DAILY_STATS.dailyIncome = (SELECT	SUM(d.totalCost)
							FROM	order_data d 
							WHERE d.orderDate = '2021-05-07'
							);

UPDATE DAILY_STATS s -- UPDATES DAILY_STATS.dailyOrderAmt attribute given a specific orderDate
SET s.dailyOrderAmt = (SELECT COUNT(*)
					  FROM ORDER_DATA d
					  WHERE d.orderDate = '2021-05-07'
						)
WHERE s.orderDate = '2021-05-07';
